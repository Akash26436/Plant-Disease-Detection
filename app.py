...
# Try to get GDRIVE_FILE_ID from streamlit secrets, fallback to env variables
try:
    GDRIVE_FILE_ID = st.secrets.get("GDRIVE_FILE_ID") or os.environ.get("GDRIVE_FILE_ID")
except Exception:
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        if not GDRIVE_FILE_ID:
            st.error("Model weights file (`best.pt`) not found locally, and no `GDRIVE_FILE_ID` was provided via secrets or environment variables.")
            st.stop()
